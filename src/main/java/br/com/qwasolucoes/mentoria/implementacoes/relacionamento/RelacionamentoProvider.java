...
public class RelacionamentoProvider implements Relacionamentos {
	...
	public List<Pessoa> buscarPessoasPorTipoContato(String tipoContato) {
		...
		for (Contato contato : listaContatos) {
			if (contato.getTipo().equals(tipoContato)) {
				Pessoa pessoa = pegaCpf.get(contato.getCpfCnpj());
				if (pessoa != null) {
					resultado.add(pessoa);
				}
			}
	...
	public List<Pessoa> buscarPessoasPorNomeBairroContem(String valor) {
		...
		Collections.sort(resultado, new Comparator<Pessoa>() {

			@Override
			public int compare(Pessoa p1, Pessoa p2) {
				return p1.getCpfCnpj().compareTo(p2.getCpfCnpj());
			}
		});
	...
	public List<Pessoa> buscarPessoasPorEscolaridadeAreaAtuacao(String areaAtuacao) {
		...
		for (Escolaridade escolaridade : listaEscolaridade) {
			for (Instituicao instituicao : listaInstituicoes) {
				if (escolaridade.getCodigoInstituicao().equals(instituicao.getAreaAtuacao())
	...
	public List<Pessoa> buscarPessoasPorEscolaridadeAnoTermino(Integer ano) {
		...
		for (Pessoa pessoa : listaPessoas) {
			for (Escolaridade escolaridade : listaEscolaridade) {
				if (escolaridade.getDataTermino().length() > 1) {
					Integer anoTermino = separaAnoData(escolaridade.getDataTermino());
					if (anoTermino.equals(ano) && pessoa.getCpfCnpj().equalsIgnoreCase(escolaridade.getCpfCnpj())) {
						resultado.add(pessoa);
					}
				}
			}
	...
	public List<Pessoa> buscarPessoasPorProfissaoAreaAtuacaoEscolaridadePorSemestre(String areaAtuacao,
		...
		for (Escolaridade escolaridade : listaEscolaridade) {
			Pessoa pessoa = filtro.get(escolaridade.getCpfCnpj());
			if (pessoa != null && !escolaridade.getSemestreAtual().isEmpty()) {
				int semestreConvertido = Integer.parseInt(escolaridade.getSemestreAtual());
				if (semestreConvertido == semestre) {
					resultado.add(pessoa);
				}
			}
		}
		...
		for (Instituicao instituicao : listaInstituicoes) {
			for (Escolaridade escolaridade : listaEscolaridade) {
				if (instituicao.getCodigo().equals(escolaridade.getCodigoInstituicao())) {
					Pessoa pessoa = parcial.get(escolaridade.getCpfCnpj());
	...
	public List<String> buscarNomeDoConjungeDasPessoasMaioresIdadeEEstadoCivil(String estadoCivil) {
		...
		List<String> resultado = new ArrayList<>();
		List<Pessoa> maiores = new ArrayList<>();
	...
	public List<Contato> buscarContatoPorProfissaoAreaAtuacao(String areaAtuacao) {
		...
		for (Contato contato : listaContatos) {
			Pessoa pessoa = parcial.get(contato.getCpfCnpj());
			if (pessoa != null) {
	...
	public List<Contato> buscarContatoPorProfissaoAreaAtuacaoEnderecoPorEstadoEBairro(String areaAtuacao, String estado,
		...
		for (Endereco endereco : listaEnderecos) {
			...
			if (pessoa != null && endereco.getBairro().equalsIgnoreCase(bairro)
					&& endereco.getEstado().equalsIgnoreCase(estado)) {
				filtro.put(pessoa.getCpfCnpj(), pessoa);
			}
		}
	...
	public List<String> buscarEmailPorPessoaAnoNascimentoProfissaoAreaAtuacaoEscolaridadePorAreaAtuacao(
		...
		for (Contato contato : listaContatos) {
			Pessoa pessoa = pessoas.get(contato.getCpfCnpj());
			if (pessoa != null && contato.getTipo().equals("3")) {
	...
	public Integer buscarQuantidadeTotalPessoasPorProfissao(String nomeProfissao) {
		...
		for (Profissao profissao : listaProfissoes) {
			if (profissao.getNomeProfissao().equalsIgnoreCase(nomeProfissao)) {
				profCod = profissao.getCodigoProfissao();
				break;
			}
		}
		...
		for (Empresa empresa : listaEmpresas) {
			Pessoa pessoa = pegaCpf.get(empresa.getCpfCnpj());
			if (pessoa != null && empresa.getCodigoProfissao().equals(profCod)) {
				resultado++;
			}
		}
	...
	public Integer buscarQuantidadeTotalPessoasPorSalarioBaseMaiorIgual(BigDecimal salarioBase) {
		...
		for (Empresa empresa : listaEmpresas) {
			...
			for (Profissao profissao : listaProfissoes) {
				...
				BigDecimal salario = new BigDecimal(profissao.getSalarioBase().replace("R$", "").replace(".", "")
	...
	public Integer buscarQuantidadeTotalPessoasPorProfissaoAreaAtuacaoEscolaridadePorSemestre(String areaAtuacao,
		...
		int resultado = 0;
		Map<String, Pessoa> pegaCpf = new HashMap<>();
		Set<String> cpfAtuacao = new HashSet<>();
		...
		for (Empresa empresa : listaEmpresas) {
			for (Profissao profissao : listaProfissoes) {
				if (profissao.getCodigoProfissao().equals(empresa.getCodigoProfissao())
						&& profissao.getAreaAtuação().equalsIgnoreCase(areaAtuacao)) {
					cpfAtuacao.add(empresa.getCpfCnpj());
				}
			}
		}
		...
		for (Escolaridade escolaridade : listaEscolaridade) {
			if (escolaridade.getSemestreAtual() != null
					&& escolaridade.getSemestreAtual().equalsIgnoreCase(semestre.toString())
					&& cpfAtuacao.contains(escolaridade.getCpfCnpj())) {
				resultado++;
			}
		}
	...
	public Integer buscarQuantidadeTotalConjugeDasPessoasMaioresIdadeEEstadoCivil(String estadoCivil) {
		...
		for (Pessoa pessoa : listaPessoas) {
			try {
				int idade = converteIdade(pessoa.getDataNascimento());
				if (pessoa.getConjuge() != null && idade >= 18) {
					resultado++;
				}
			} catch (ParseException e) {
				e.getMessage();
	...
	public Integer buscarQuantidadeTotalPessoasPorBairroPorProfissaoPorAreaAtuacao(String bairro,
		...
		for (Profissao profissao : listaProfissoes) {
			for (Empresa empresa : listaEmpresas) {
				...
				if (pessoa != null && profissao.getCodigoProfissao().equals(empresa.getCodigoProfissao())
						&& profissao.getAreaAtuação().equalsIgnoreCase(areaAtuacaoProfissao)) {
